...
class Video_game:
    ...
    def save(cls, data:dict):
        query = "INSERT INTO video_games (title, synopsis, rating, used, image, created_at, updated_at, user_id) VALUES ( %(title)s, %(synopsis)s, %(rating)s, %(used)s, %(image)s, %(user_id)s , NOW() , NOW() );"
        return connectToMySQL(DATABASE).query_db(query, data)

    @classmethod
    def get_one(cls, data:dict):
        query = "SELECT * FROM video_games WHERE id=%(id)s;"
    ...
    def get_all(cls, data:dict):
        query = "SELECT * FROM video_games WHERE user_id = %(user_id)s;"
    ...
    def update(cls, data:dict):
        query = 'UPDATE video_games SET title=%(title)s, synopsis=%(synopsis)s, rating=%(rating)s, used=%(used)s, image=%(image)s, updated_at=NOW() WHERE (id=%(id)s);'
        return connectToMySQL(DATABASE).query_db(query, data)


    @classmethod
    def delete(cls, data):
        query = 'DELETE FROM video_games WHERE (id=%(id)s);'
        return connectToMySQL(DATABASE).query_db(query, data)
