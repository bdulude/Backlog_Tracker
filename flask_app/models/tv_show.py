...
class Tv_show:
    ...
    def save(cls, data:dict):
        query = "INSERT INTO tv_shows (title, synopsis, rating, used, image, created_at, updated_at, user_id) VALUES ( %(title)s, %(synopsis)s, %(rating)s, %(used)s, %(image)s, %(user_id)s , NOW() , NOW() );"
    ...
    def get_all(cls, data:dict):
        query = "SELECT * FROM tv_shows WHERE user_id = %(user_id)s;"
    ...
    def update(cls, data:dict):
        query = 'UPDATE tv_shows SET title=%(title)s, synopsis=%(synopsis)s, rating=%(rating)s, used=%(used)s, image=%(image)s, updated_at=NOW() WHERE (id=%(id)s);'
        return connectToMySQL(DATABASE).query_db(query, data)
